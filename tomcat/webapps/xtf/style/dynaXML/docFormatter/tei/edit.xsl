<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:key name="index" match="//*:ref" use="@target"/>
  <xsl:key name="index1" match="//*:ref" use="@subtype"/>
  <xsl:key name="quote" match="//*:quote" use=" substring(@corresp, 2)"/>

  <xsl:variable name="entry"
    select="document (concat ($index.path,'/index.xml'))//*[*:listPerson|*:listPlace|*:list[@type='gloss']|*:listBibl[@type='bibl_other']]"/>
  ...
  <xsl:variable name="content.href"><xsl:value-of select="$query.string"
      ...
      />&#038;toc.depth=<xsl:value-of select="$toc.depth"/>&#038;brand=<xsl:value-of select="$brand"
      />&#038;anchor.id=<xsl:value-of select="$anchor.id"/><xsl:value-of select="$search"
    /></xsl:variable>
  ...
  <xsl:template name="status">
    ...
    <h2>Edition status <!--<xsl:variable name="red">
        <xsl:choose>
    ...
    <table width="75%" border="0">
      <tr>
        <td/>
        <td>
          <img src="{$icon.path}text.png" align="top" title="Plain text"/>
        </td>

        <xsl:if test="//*:editorialDecl//*:item/@n='div'">
          <td>
            <img src="{$icon.path}structure.png" align="top" title="Text hierarchy"/>
          </td>
        </xsl:if>

        <xsl:if test="//*:editorialDecl//*:item/@n='typoAbb'">
          <td>
            <img src="{$icon.path}N.png" align="top"
              title="Normalization of typography and resolution of abbreviations"/>
          </td>
        </xsl:if>

        <xsl:if test="//*:editorialDecl//*:item/@n='addDelCorr'">
          <td>
            <img src="{$icon.path}correct.png" align="top"
              title="Addidtions, Deletions, Corrections"/>
          </td>
        </xsl:if>

        <xsl:if test="//*:editorialDecl//*:item/@n='music'">
          <td>
            <img src="{$icon.path}music.png" align="top" title="Musical examples"/>
          </td>
        </xsl:if>

        <xsl:if test="//*:editorialDecl//*:item/@n='pPWM'">
          <td>
            <img src="{$icon.path}ppwt.png" align="top"
              title="Persons, places, works and musical terminology"/>
          </td>
        </xsl:if>

        <xsl:if test="//*:editorialDecl//*:item/@n='citations'">
          <td>
            <img src="{$icon.path}citation.png" align="top" title="Citations"/>
          </td>
        </xsl:if>
      </tr>
    </table>
  ...
  <xsl:template name="generalis">
    ...
    <h3>
      <span class="spoilerDiv" onclick="ouvrirFermerSpoiler('generalis')">
        <img src="{$icon.path}i_expand.gif" onclick="changeimg(this)"/>
      </span>
      ...
      <xsl:choose>
        ...
        <xsl:otherwise> <xsl:value-of> Index Generalis</xsl:value-of></xsl:otherwise>
      </xsl:choose>
    ...
    <div id="generalis" style="display: none">
      <xsl:for-each select="$entry/*/*[parent::*:listPerson|parent::*:listPlace|parent::*:listBibl]">
        ...
        <xsl:if test="$xmlExists='true'">
          <hr/>
          <!-- Level I -->
          <table cellpadding="0" cellspacing="0" class="title" border="0">
            <tr>
              <td width="5"/>
              <td>
                <!-- persons -->
                            ...
                            <xsl:choose>
                              ...
                              <xsl:otherwise>
                                <a href="{$link}" target="_blank"
                                  style="color:brown; text-decoration: none">
                                  <xsl:value-of select="$loc"/>
                                </a>
                              </xsl:otherwise>
                            </xsl:choose>
  ...
  <xsl:template name="exempla">
    ...
    <h3>
      ...
      <xsl:choose>
        ...
        <xsl:otherwise> <xsl:value-of> Exempla</xsl:value-of></xsl:otherwise>
      </xsl:choose>
    </h3>
                  ...
                  <tr>
                    ...
                    <td>
                      <xsl:for-each select="*:biblScope">
                        ...
                        <xsl:choose>
                          <xsl:when
                            test="@type='pp' or @type='lib' or @type='cap' or @type='fol' or @type='num' or @type='ll'">
                            <xsl:variable name="p" select="text()"/>
                            <xsl:variable name="type">
                              <xsl:call-template name="biblType"/>
                            </xsl:variable>
                                ...
                                <xsl:if test="substring (@corresp,2)">
                                  ...
                                  <a target="content">
                                    <xsl:attribute name="href">
                                      ...
                                      /></xsl:attribute>
                                    <img src="{$icon.path}arrow.gif" width="14" height="10"
                                      border="0" alt="arrow"/>
                                  </a>
  ...
  <xsl:template name="thesaurus">
    ...
    <h3>
      ...
      <xsl:choose>
        ...
        <xsl:otherwise> <xsl:value-of> Thesaurus</xsl:value-of></xsl:otherwise>
      </xsl:choose>
  ...
  <xsl:template name="localentry" exclude-result-prefixes="#all">
                ...
                <a target="content">
                  <xsl:attribute name="href">
                    <xsl:value-of select="$doc.path"
                      ...
                      select="$brand"/>&#038;anchor.id=<xsl:value-of select="$anchor"
                      />&#038;<xsl:call-template name="create.anchor"/></xsl:attribute>
                  <xsl:if test="$anchor.id=$anchor">
                    <xsl:attribute name="id">
                      <xsl:value-of select="$anchor"/>
                    </xsl:attribute>
                  </xsl:if>
                  <img src="{$icon.path}arrow.gif" width="14" height="10" border="0" alt="arrow"/>
                </a>
  ...
  <xsl:template name="createPath">
    ...
    <xsl:attribute name="onclick">
      <xsl:text>javascript:window.open('</xsl:text><xsl:value-of select="$xtfURL"
        ...
        />index.xml&#038;doc.view=popup&#038;chunk.id=<xsl:value-of select="$pXMLID"
  ...
  <xsl:template name="createLinkToOccurrence">
    <xsl:if test="not(@subtype='#editor')">
      ...
      <a target="content">
        <xsl:attribute name="href">
          <xsl:value-of select="$doc.path"/>&#038;doc.view=content&#038;chunk.id=<xsl:value-of
            select="$refchunk"/>&#038;toc.depth=<xsl:value-of select="$toc.depth"
            />&#038;brand=<xsl:value-of select="$brand"/>&amp;#<xsl:value-of select="$anchor"
          /></xsl:attribute>
        <img src="{$icon.path}arrow.gif" width="14" height="10" border="0" alt="arrow"/>
      </a>
  ...
  <xsl:template name="createBiblioReference">
    ...
    <xsl:choose>
      <xsl:when test="$link">
        <a style="color:brown; text-decoration: none" href="{$link}" target="_blank">
          <i>
            <xsl:choose>
  ...
  <xsl:template name="data">
    ...
    <table>
      <tr>
        ...
        <td>
          <a href="{$doc.adress}" target="_blanc" style="color:brown; text-decoration: none">
      ...
      <tr>
        ...
        <td>
          <a href="{concat ($index.path,'index.xml')}" target="_blanc"
            style="color:brown; text-decoration: none">
