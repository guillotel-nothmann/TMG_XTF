...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xtf="http://cdlib.org/xtf"
    ...
    <xsl:param name="abb"
        select="if (normalize-space(session:getData('abb')) != '') then session:getData('abb') else 'false'"/>
    <xsl:param name="orig"
        select="if (normalize-space(session:getData('orig')) != '') then session:getData('orig') else 'false'"/>
    <xsl:param name="pc"
        select="if (normalize-space(session:getData('pc')) != '') then session:getData('pc') else 'false'"/>
    <xsl:param name="lb"
        select="if (normalize-space(session:getData('lb')) != '') then session:getData('lb') else 'false'"/>
    <xsl:param name="names"
        select="if (normalize-space(session:getData('names')) != '') then session:getData('names') else 'true'"/>
    <xsl:param name="works"
        select="if (normalize-space(session:getData('works')) != '') then session:getData('works') else 'true'"/>
    <xsl:param name="places"
        select="if (normalize-space(session:getData('places')) != '') then session:getData('places') else 'true'"/>
     <xsl:param name="thesaurus"
        select="if (normalize-space(session:getData('thesaurus')) != '') then session:getData('thesaurus') else 'false'"/>
    <xsl:param name="internal"
        select="if (normalize-space(session:getData('internal')) != '') then session:getData('internal') else 'true'"/>
    <xsl:param name="add"
        select="if (normalize-space(session:getData('add')) != '') then session:getData('add') else 'false'"/>
    <xsl:param name="del"
        select="if (normalize-space(session:getData('del')) != '') then session:getData('del') else 'false'"/>
    <xsl:param name="corr"
        select="if (normalize-space(session:getData('corr')) != '') then session:getData('corr') else 'false'"/>
    <xsl:param name="sic"
        select="if (normalize-space(session:getData('sic')) != '') then session:getData('sic') else 'false'"/>
    
    <xsl:param name="musicEmend"
        select="if (normalize-space(session:getData('musicEmend')) != '') then session:getData('musicEmend') else 'false'"/>
    
    <xsl:param name="musicOriginal"
        select="if (normalize-space(session:getData('musicOriginal')) != '') then session:getData('musicOriginal') else 'false'"/>
    ...
    <xsl:function name="editURL:set">
        ...
        <xsl:choose>
            <xsl:when test="matches($url, $regex)">
                <xsl:value-of
                    select="editURL:clean(replace($url, $regex, concat(';', $param, '=', $protectedValue, ';')))"
                />
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of
                    select="editURL:clean(concat($url, ';', $param, '=', $protectedValue))"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:function>
    ...
    <xsl:function name="editURL:protectValue">
        <xsl:param name="value"/>
        <xsl:value-of
            select="replace(replace(replace(replace(replace(replace($value,
            '%',     '%25'),
            '[+]',   '%2B'),
            '&amp;', '%26'), 
            ';',     '%3B'), 
                ...
                <div>
                    ...
                    <form action="/xtf/search" method="get">
                        <input type="radio" name="lang" value="en">
                            <xsl:if test="$lang='en'"><xsl:attribute name="checked"
                                    select="'checked'"/></xsl:if>
                            <xsl:text>Engish</xsl:text>
                        </input>&#160;&#160; <input type="radio" name="lang" value="fr">
                            <xsl:if test="$lang='fr'"><xsl:attribute name="checked"
                                    select="'checked'"/></xsl:if>
                            <xsl:text>French</xsl:text>
                        </input>&#160; <input type="radio" name="lang" value="de">
                            <xsl:if test="$lang='de'"><xsl:attribute name="checked"
                                    select="'checked'"/></xsl:if>
    ...
    <xsl:template name="setLang">
        ...
        <html xml:lang="fr" lang="fr">
            ...
            <body
                onload="autoCloseTimer = setTimeout('window.close()', 1000); opener.top.location.reload();">
                ...
                <b>
                    ...
                    <xsl:value-of
                        select="if ($lang='sp') then 'Spanish' 
                ...
                <div>
                    ...
                    <form action="/xtf/search" method="get">
                        <table border="0" width="100%">
                            <tr><td colspan="3"><b>Regularization and Normalization:</b></td></tr>
                            <tr>
                                <td width="5%"></td>
                                <td width="75%">

                                    <input type="hidden" name="abb" value="false"/>
                                    <input type="checkbox" name="abb" value="true"> 
                            ...
                            <tr>
                                <td width="5%"></td>
                                <td width="75%">
                                    <input type="hidden" name="orig" value="false"/>
                                    <input type="checkbox" name="orig" value="true"> 
                            ...
                            <tr>
                                <td width="5%"></td>
                                <td width="75%">
                                    <input type="hidden" name="pc" value="false"/>
                            ...
                            <tr>
                                <td width="5%"></td>
                                <td width="75%">
                                    <input type="hidden" name="lb" value="false"/>
                                    <input type="checkbox" name="lb" value="true">
                                        <xsl:if test="$lb='true'">
                                            <xsl:attribute name="checked" select="'checked'"/>
                                        </xsl:if>
                            ...
                            <tr><td colspan="3"><b>Additions, Deletions and Corrections:</b></td></tr>
                            <tr>
                                <td width="5%"></td>
                                <td width="75%">
                            ...
                            <tr>
                                ...
                                <td width="75%">
                                    ...
                                    <input type="checkbox" name="del" value="true">
                            ...
                            <tr>
                                <td width="5%"></td>
                                <td width="75%">
                                    <input type="hidden" name="corr" value="false"/>
                                    <input type="checkbox" name="corr" value="true">
                            ...
                            <tr>
                                <td width="5%"></td>
                                <td width="75%">
                                    <input type="hidden" name="sic" value="false"/>
                                    <input type="checkbox" name="sic" value="true">
                            ...
                            <tr><td colspan="3"><b>Hyperlinks:</b></td></tr>
                            <tr>
                                <td width="5%"></td>
                                <td width="75%">
                                    <input type="hidden" name="names" value="false"/>
                                    <input type="checkbox" name="names" value="true">
                            ...
                            <tr>
                                <td width="5%"></td>
                                <td width="75%">
                                    <input type="hidden" name="works" value="false"/>
                                    <input type="checkbox" name="works" value="true">
                            ...
                            <tr>
                                <td width="5%"></td>
                                <td width="75%">
                                    <input type="hidden" name="places" value="false"/>
                                    <input type="checkbox" name="places" value="true">
                            ...
                            <tr>
                                ...
                                <td width="75%">
                                    ...
                                    <input type="checkbox" name="thesaurus" value="true">
                            ...
                            <tr>
                                ...
                                <td width="20%">
                                    ...
                                    <input type="hidden" name="smode" value="setEdit"/>
                                </td>
                            </tr> 
                        </table>
                    </form>
    ...
    <xsl:template name="setEdit">
        ...
        <html xml:lang="en" lang="en">
            ...
            <body
                onload="autoCloseTimer = setTimeout('window.close()', 5000); opener.top.location.reload();">

                <xsl:copy-of select="$brand.header"/>

                <h1>Edition Set</h1>
                <b><xsl:text>Edition parameters have been set to :</xsl:text></b>
                    ...
                    <table width="100%" border="0">
                        ...
                        <tr>
                            ...
                            <td width="20%">
                                <xsl:value-of select="if ($abb='true') then 'true' else 'false'"/>
                            </td>
                        </tr>
                        <tr>
                            <td width="5%"></td>
                            ...
                            <td width="20%">
                                <xsl:value-of select="if ($orig='true') then 'true' else 'false'"/>
                            </td>
                        </tr>
                        <tr>
                            <td width="5%"></td>
                            ...
                            <td width="20%">
                                <xsl:value-of select="if ($pc='true') then 'true' else 'false'"/>
                            </td>
                        </tr>
                        <tr>
                            <td width="5%"></td>
                            ...
                            <td width="20%">
                                <xsl:value-of select="if ($lb='true') then 'true' else 'false'"/>
                            </td>
                        </tr>
                        <tr>
                            <td width="5%"></td>
                            ...
                            <td width="20%">
                                <xsl:value-of select="if ($musicOriginal='true') then 'true' else 'false'"/>
                            </td>
                        </tr>
                        
                        
                        <tr><td colspan="3"><b>Additions, Deletions, Corrections and Errors:</b></td></tr>
                        <tr>
                            <td width="5%"></td>
                            <td width="75%">Highlight additions:</td>
                            <td width="20%">
                                <xsl:value-of select="if ($add='true') then 'true' else 'false'"/>
                            </td>
                        </tr>
                        <tr>
                            <td width="5%"></td>
                            ...
                            <td width="20%">
                                <xsl:value-of select="if ($del='true') then 'true' else 'false'"/>
                            </td>
                        </tr>
                        <tr>
                            <td width="5%"></td>
                            ...
                            <td width="20%">
                                <xsl:value-of select="if ($corr='true') then 'true' else 'false'"/>
                            </td>
                        </tr>
                        <tr>
                            <td width="5%"></td>
                            ...
                            <td width="20%">
                                <xsl:value-of select="if ($sic='true') then 'true' else 'false'"/>
                            </td>
                        </tr>
                        <!--<tr>
                            <td width="5%"></td>
                        ...
                        <tr>
                            ...
                            <td width="20%">
                                <xsl:value-of select="if ($names='true') then 'true' else 'false'"/>
                            </td>
                        </tr>
                        <tr>
                            <td width="5%"></td>
                            ...
                            <td width="20%">
                                <xsl:value-of select="if ($works='true') then 'true' else 'false'"/>
                            </td>
                        </tr>
                        <tr>
                            <td width="5%"></td>
                            ...
                            <td width="20%">
                                <xsl:value-of select="if ($places='true') then 'true' else 'false'"/>
                            </td>
                        </tr>
                        <tr>
                            <td width="5%"></td>
                            ...
                            <td width="20%">
                                <xsl:value-of select="if ($thesaurus='true') then 'true' else 'false'"/>
                            </td>
                        </tr>
                        <tr>
                            <td width="5%"></td>
